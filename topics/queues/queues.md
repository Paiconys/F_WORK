...
1. [Celery «зачем»](#celery-зачем)
2. [Queue — зачем очередь (фон: ресайз, письма, антиспам)](#queue--зачем-очередь-фон-ресайз-письма-антиспам)
...
#### Коротко
Очередь забирает долгую работу из запроса пользователя. 

#### Развёрнуто
HTTP отвечает «принято», воркер делает тяжелое позже и с ретраями. Кандидаты: ресайз, вирусопроверка, нотификации. Без очереди пиковая нагрузка бьёт latency API.
