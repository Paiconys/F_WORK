# Lambda в цикле — late binding

## Коротко

Печатает `2 2 2`: lambda захватила имя `i`, к вызову i уже 2.

## Развёрнуто

Фикс: `lambda i=i: print(i)` — дефолт фиксирует значение на итерации.

## Пример

Код: [`lambda_late_binding.py`](./examples/lambda_late_binding.py)

```python
"""

Dependencies: none (stdlib)
DRILL: late binding — lambda в цикле."""

flist = []

for i in range(3):
 # Захватывается ИМЯ i, не значение на итерации
 flist.append(lambda: print(i))

[f() for f in flist] # 2 2 2

# Фикс: дефолт фиксирует значение
flist2 = []
for i in range(3):
 flist2.append(lambda i=i: print(i))

[f() for f in flist2] # 0 1 2
```
